# ShadowOps 一页式 PRD（v0.1）

> 状态：已确认（2026-08-24）｜首版范围：PostgreSQL + Alembic Migration + 本地 Git 仓库 + Docker 影子环境
...
1. 首版交互面采用 **CLI + 本地 Web 报告/审批页**，不先做完整 Web IDE。
2. 默认审计范围采用 **工作区/暂存区相对 HEAD，并允许显式 `base..head`**。
3. 测试数据采用 **仓库 fixture 优先、确定性合成数据兜底**，首版不复制生产数据。
4. 高风险审批表示 **批准审计结论/允许团队继续人工流程**，ShadowOps 不执行合并或上线。
5. MVP 固定单个 PostgreSQL 主版本；架构阶段选定 PostgreSQL 16，镜像 patch/digest 在实现锁文件中固定。
